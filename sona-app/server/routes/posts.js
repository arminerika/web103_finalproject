...
router.patch('/:id', async (req, res) => {
    ...
    if (!(await isAdminOf(user_id, artist_id))) {
        return res.status(403).json({ error: "Not authorized to edit a post this artist" });
    }
...
router.delete('/:id', async (req, res) => {
    ...
    if (!(await isAdminOf(user_id, artist_id))) {
        return res.status(403).json({ error: "Not authorized to delete a post this artist" });
    }
